...
## 系统架构设计
### 总体架构设计
我们将系统拆分为 **用户服务、权限服务、日志服务** 四个核心微服务，并依赖一套标准化的基础架构层（Nacos, RocketMQ等）。通过 API 网关对外提供统一、安全的入口。
...
### **接口设计约定**
+ **Base URL**:  `user-service` 的基础路径是 `/api/user`，`permission-service` 的基础路径是 `/api/permission`。
...
| **6. 查询指定用户信息** | `GET` | `/users/{userId}` | _无_ (userId从路径获取) | 本人, 管理员, 超管 | `{ "code": 200, "message": "Success", "data": { "userId": 456, "username": "anotherUser", ... } }` |
| **7. 修改用户信息** | `PUT` | `/users/{userId}` | **Body**: `{ "email": "new.email@example.com", "phone": "139..." }` | 本人, 管理员, 超管 | `{ "code": 200, "message": "用户信息修改成功", "data": { ...updated user object... } }` |
| **8. 修改密码** | `PUT` | `/password` | **Body**: `{ "oldPassword": "string", "newPassword": "string" }` | 本人 | `{ "code": 200, "message": "密码修改成功", "data": null }` |
| **9. 重置密码** | `POST` | `/users/{userId}/reset-password` | _无_ | 管理员, 超管 | `{ "code": 200, "message": "密码已重置为: <new_random_password>", "data": { "newPassword": "<new_random_password>" } }` |


 ---

### **二、 权限服务 (Permission Service)**
**服务基础路径**: `/api/permission`
...
| **3. 查询用户角色 (内部)** | `GET` | `/internal/users/{userId}/role` | _无_ (userId从路径获取) | 系统内部 | `{ "code": 200, "message": "Success", "data": { "roleCode": "admin" } }` |
| **4. 升级用户角色** | `PUT` | `/users/{userId}/upgrade-to-admin` | _无_ | 超级管理员 | `{ "code": 200, "message": "用户角色已升级为管理员", "data": { "userId": 456, "newRole": "admin" } }` |
| **5. 降级用户角色** | `PUT` | `/users/{userId}/downgrade-to-user` | _无_ | 超级管理员 | `{ "code": 200, "message": "用户角色已降级为普通用户", "data": { "userId": 789, "newRole": "user" } }` |


#### **接口设计思考**:
+ **资源化**: `GET /users`, `GET /users/{userId}`, `PUT /users/{userId}` 都体现了对 `user` 资源的增删改查。
+ **动词与名词**: HTTP方法（GET, POST, PUT, DELETE）是动词，URL路径尽量使用名词（资源）。对于无法清晰映射到CRUD的操作，如“重置密码”，可以使用子资源或动作形式，如 `/users/{userId}/reset-password`，这比 `/resetPassword` 更符合REST风格。
+ **内部与外部隔离**: 使用 `/internal` 路径前缀明确标记了服务间调用的接口，便于在网关层设置访问控制策略，防止其被外部直接调用。
+ **参数传递**:
...
-- 结构与 user_db_0 中的 users_0 和 users_1 完全相同
CREATE TABLE users_0 ( ... );
CREATE TABLE users_1 ( ... );
```
...
##### **基本流程 (Happy Path)**
1. **API 请求**: 客户端向 API 网关发送一个 `POST` 请求，该请求被路由到 `user-service` 的 `/api/user/register` 端点。请求体是一个 JSON 对象: `{ "username": "string", "password": "string", "email": "string" }`。
...
5. **为角色绑定进行 RPC 调用 (**`user-service`** -> **`permission-service`**)**:
    ...
    - 请求被发送到 `permission-service` 的 `/api/permission/internal/roles/bind-default` 端点，并携带新的 `userId`。
...
##### **基本流程 (Happy Path)**
1. **API 请求**: 客户端发送一个 `POST` 请求到 `/api/user/login`，携带 `{ "username": "string", "password": "string" }`。
2. **用户检索 (**`user-service`**)**:
    - 服务接收到请求。
...
##### **基本流程 (Happy Path)**
1. **API 请求**: 客户端发送一个 `POST` 请求到 `/api/user/logout`，并携带一个有效的 `Authorization: Bearer <jwt_token>` 请求头。
...
##### **基本流程 (Happy Path)**
1. **API 请求**: 客户端发送一个 `GET` 请求到 `/api/user/me`，并在请求头中携带一个有效的 JWT。
...
##### **基本流程 (Happy Path)**
1. **API 请求**: 一个管理员客户端发送一个 `GET` 请求到 `/api/user/users`，带有查询参数如 `?page=1&size=10&sort=gmt_create,desc&username=test` 和一个有效的 JWT。
2. **认证和授权 (**`user-service`**)**:
    - 安全过滤器验证 **JWT** 并提取操作者的 `userId`。
    - 为了检查权限，`user-service` 通过 **OpenFeign** 向 `permission-service` 的 `/internal/users/{userId}/role` 端点发起 RPC 调用，该端点通过 **Nacos** 进行服务发现。
...
##### **基本流程 (Happy Path)**
1. **API 请求**: 客户端发送一个 `GET` (用于查询) 或 `PUT` (用于修改) 请求到 `/api/users/{userId}`。`PUT` 请求包含一个带有待更新字段的请求体。需要一个有效的 JWT。
2. **认证和授权 (**`user-service`**)**:
...
##### **基本流程 (Happy Path)**
1. **API 请求**:
    - **修改**: `PUT` 到 `/api/user/password`，请求体为 `{ "oldPassword": "...", "newPassword": "..." }`。
    - **重置**: `POST` 到 `/api/user/users/{userId}/reset-password`。
2. **认证和授权 (**`user-service`**)**:
...
1. **API 请求 (**`permission-service`**)**: 服务通过 **OpenFeign** 从 `user-service` 接收一个内部请求。
    - **绑定**: `POST`到 `/api/permission/internal/roles/bind-default`，携带一个 `userId`。
    - **查询**: `GET`到 `/api/permission/internal/users/{userId}/role`。
...
##### **基本流程 (Happy Path)**
1. **API 请求 (**`permission-service`**)**: 一个超级管理员通过前端，导致一个请求被发送到 API 网关，并路由到 `permission-service`。
    - **升级**: `PUT` 到 `/api/permission/users/{userId}/upgrade-to-admin`。
    - **降级**: `PUT` 到 `/api/permission/users/{userId}/downgrade-to-user`。
2. **认证**: 请求必须通过一个安全过滤器，该过滤器验证 JWT 并确认操作者拥有 'super_admin' 角色。（这意味着 `permission-service` 也有一个检查角色的机制，或者它信任 API 网关来强制执行此操作）。
